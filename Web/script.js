...
async function login() {
  ...
  try {
    const response = await fetch("https://localhost:7267/api/Cuentas/VerificarCredenciales", {
      method: "POST",
      headers: {
    ...
    if (response.ok) {
      const data = await response.text() // si tu API solo devuelve texto
      ...
      errorDiv.textContent = errorText || "Credenciales incorrectas"
